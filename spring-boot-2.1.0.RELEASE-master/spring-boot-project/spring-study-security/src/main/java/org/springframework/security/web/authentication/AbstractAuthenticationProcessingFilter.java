 ...
 *
 * The filter requires that you set the <tt>authenticationManager</tt> property.
 * An <tt>AuthenticationManager</tt> is required to process the authentication
 * request tokens created by implementing classes.
 * <p>
 ...
 * will be placed into the <code>SecurityContext</code> for the current thread,
 * which is guaranteed to have already been created by an earlier filter.
 * <p>
 * The configured
 * {@link #setAuthenticationSuccessHandler(AuthenticationSuccessHandler)
 * AuthenticationSuccessHandler} will then be called to take the redirect to the
 * appropriate destination after a successful login. The default behaviour is
 * implemented in a {@link SavedRequestAwareAuthenticationSuccessHandler} which
 * will make use of any <tt>DefaultSavedRequest</tt> set by the
 * <tt>ExceptionTranslationFilter</tt> and redirect the user to the URL
 * contained therein. Otherwise it will redirect to the webapp root "/". You can
 * customize this behaviour by injecting a differently configured instance of
 * this class, or by using a different implementation.
 * <p>
 * See the
 ...
 * {@link AuthenticationFailureHandler} to allow the failure information to be
 * conveyed to the client. The default implementation is
 * {@link SimpleUrlAuthenticationFailureHandler} , which sends a 401 error code
 * to the client. It may also be configured with a failure URL as an
 * alternative. Again you can inject whatever behaviour you require here.
 *
 * <h4>Event Publication</h4>
 *
 * If authentication is successful, an
 * {@link InteractiveAuthenticationSuccessEvent} will be published via the
 * application context. No events will be published if authentication was
 * unsuccessful, because this would generally be recorded via an
 * {@code AuthenticationManager}-specific application event.
 *
 * <h4>Session Authentication</h4>
 *
 * The class has an optional {@link SessionAuthenticationStrategy} which will be
 * invoked immediately after a successful call to
 * {@code attemptAuthentication()}. Different implementations
 * {@link #setSessionAuthenticationStrategy(SessionAuthenticationStrategy) can
 * be injected} to enable things like session-fixation attack prevention or to
 * control the number of simultaneous sessions a principal may have.
 *
 * @author Ben Alex
 * @author Luke Taylor
 */
public abstract class AbstractAuthenticationProcessingFilter extends GenericFilterBean
	 ...
	 *
	 * @param requiresAuthenticationRequestMatcher the {@link RequestMatcher} used
	 *                                             to determine if authentication is
	 ...
	 * {@link #attemptAuthentication(HttpServletRequest, HttpServletResponse)
	 * attemptAuthentication} will be invoked to perform the authentication. There
	 ...
	 * received by this method where the returned <tt>Authentication</tt> object is
	 * not null.
	 * </ol>
	 */
	public void doFilter(ServletRequest req, ServletResponse res, FilterChain chain)
		...
		try {
			// 具体实现在它的子类UsernamePasswordAuthenticationFilter
			...
			// 用来处理认证失败事宜，1.从securityCcontextholder中清除数据
			// 2.清除cookie的数据 3.调用认证失败的方法
			unsuccessfulAuthentication(request, response, failed);
		...
		// 用来处理认证成功事宜，1.向securityContextHolder中存入用户信息
		// 2.处理cookie；3.发布认证成功事件，事件类型为InteractiveAuthenticationSuccessEvent
		// 表示通过一些自动交互的方式认证成功，例如通过rememberMe的方式登录，4.调用认证成功的回调方法
		successfulAuthentication(request, response, chain, authResult);
	}

	/**
	 * Indicates whether this filter should attempt to process a login request for
	 * the current invocation.
	 * <p>
	 * It strips any parameters from the "path" section of the request URL (such as
	 * the jsessionid parameter in
	 ...
	 * progress. Before returning, the implementation should perform any additional
	 * work required to complete the process.</li>
