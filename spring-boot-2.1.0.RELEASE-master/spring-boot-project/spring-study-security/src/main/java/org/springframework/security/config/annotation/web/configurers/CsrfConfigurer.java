...
/**
 * 从上面可知，第一个安全配置是针对CSRF（跨站点请求伪造：Cross-Site Request
 * Forgery）的。一般来讲，为了防御CSRF攻击主要有三种策略：验证 HTTP Referer 字段； 在请求地址中添加 token 并验证；在
 * HTTP 头中自定义属性并验证。
 * 
 * 作者：一根线条 链接：http://events.jianshu.io/p/1a92e62fb3e4 来源：简书
 * 著作权归作者所有。商业转载请联系作者获得授权，非商业转载请注明出处。 Adds
 ...
 *
 * <ul>
 * <li>{@link ExceptionHandlingConfigurer#accessDeniedHandler(AccessDeniedHandler)}
...
public final class CsrfConfigurer<H extends HttpSecurityBuilder<H>>
	 ...
	 * {@link HttpSessionCsrfTokenRepository} wrapped by
	 * {@link LazyCsrfTokenRepository}.
	 *
	 ...
	 * applied. The default is to ignore GET, HEAD, TRACE, OPTIONS and process all
	 * other requests.
	 *
	...
	/**
	 * <p>
	 * Allows specifying {@link HttpServletRequest} that should not use CSRF
	 * Protection even if they match the
	 * {@link #requireCsrfProtectionMatcher(RequestMatcher)}.
	 * </p>
	 *
	 * <p>
	 ...
	 * <li>We also explicitly state to ignore any request that starts with
	 * "/sockjs/"</li>
	 * </ul>
	 *
	 * <pre>
	...
	/**
	 * <p>
	 * Allows specifying {@link HttpServletRequest}s that should not use CSRF
	 * Protection even if they match the
	 * {@link #requireCsrfProtectionMatcher(RequestMatcher)}.
	 * </p>
	 *
	 ...
	 * <ul>
	 * <li>Any GET, HEAD, TRACE, OPTIONS (this is the default)</li>
	 * <li>We also explicitly state to ignore any request that has a
	 * "X-Requested-With: XMLHttpRequest" header</li>
	 * </ul>
	 *
	 * <pre>
	 * http
	...
	public void configure(H http) throws Exception {
		...
		if (sessionConfigurer != null) {
			sessionConfigurer
					.addSessionAuthenticationStrategy(new CsrfAuthenticationStrategy(this.csrfTokenRepository));
		}
	 ...
	 * {@link #requireCsrfProtectionMatcher(RequestMatcher)} and any
	 * {@link #ignore()}.
	 *
	 ...
	 * {@link SessionManagementConfigurer#getInvalidSessionStrategy()} or null if
	 * not available.
	 *
	 * @param http the {@link HttpSecurityBuilder}
	...
	/**
	 * Allows registering {@link RequestMatcher} instances that should be ignored
	 * (even if the {@link HttpServletRequest} matches the
	...
	/**
	 * An {@link IgnoreCsrfProtectionRegistry} that allows optionally configuring
	 * the {@link MvcRequestMatcher#setMethod(HttpMethod)}
	 *
	 * @author Rob Winch
	 */
