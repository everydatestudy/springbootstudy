 ...
 * <li>{@link DefaultLoginPageGeneratingFilter} - if present will be populated
 * with information from the configuration</li>
 * </ul>
 * 概述 介绍 作为一个配置HttpSecurity的SecurityConfigurer,FormLoginConfigurer的配置任务如下 :
 * 
 * 配置如下安全过滤器Filter
 * 
 * UsernamePasswordAuthenticationFilter 创建的共享对象
 * 
 * AuthenticationEntryPoint FormLoginConfigurer使用到的共享对象有 :
 * 
 * AuthenticationManager RememberMeServices SessionAuthenticationStrategy
 * DefaultLoginPageGeneratingFilter FormLoginConfigurer允许使用者做如下配置 :
 * 
 * 设置登录页面URL – #loginPage
 * 
 * 缺省值为 /login。 该方法没有被调用，并且使用了WebSecurityConfigurerAdapter时，会产生一个缺省的登录页面在缺省登录URL
 * /login上。
 ...
 * 
 * 必须发起HTTP POST请求； 必须提交到登录提交处理URL,也就是#createLoginProcessingUrlMatcher所设置；
 * 用户名字段名称使用#usernameParameter所设置值； 密码字段名称使用#passwordParameter所设置值； 设置登录提交处理URL
 ...
 * 
 * 缺省值 : password ———————————————— 版权声明：本文为CSDN博主「安迪源文」的原创文章，遵循CC 4.0
 * BY-SA版权协议，转载请附上原文出处链接及本声明。
 * 原文链接：https://blog.csdn.net/andy_zhang2007/article/details/91357155
 * 
 * @author Rob Winch
...
public final class FormLoginConfigurer<H extends HttpSecurityBuilder<H>> extends
	 ...
	 * {@link WebSecurityConfigurerAdapter} a default login page will be generated
	 ...
	 * create a form that submits a request with the following requirements to work
	 * with {@link UsernamePasswordAuthenticationFilter}:
	 * </p>
	 *
	 * <ul>
	 ...
	 * rules above are followed. Below is an example login.jsp that can be used as a
	 * quick start when using JSP's or as a baseline to translate into another view
	 * technology.
	 *
	 * <pre>
	 ...
	 *
	 * Updating this value, also impacts a number of other default values. For
	 * example, the following are the default values when only formLogin() was
	 * specified.
	 *
	 * <ul>
	 ...
	 * <li>/authenticate POST - process the credentials and if valid authenticate
	 * the user</li>
	 * <li>/authenticate?error GET - redirect here for failed authentication
	 * attempts</li>
	 * <li>/authenticate?logout GET - redirect here after successfully logging
	 * out</li>
	 * </ul>
	 *
	 *
