 ...
 * </p>
 * 以上便是Spring
 * Security中进行Csrf保护的大致处理过程，其思路就是生成一个csrf的token并保存到session中，
 * 然后从每次的请求中得到携带的csrf的token并与session中保存的做比对，如果不一致则验证不通过。
 * 
...
public final class CsrfFilter extends OncePerRequestFilter {
	 ...
	 * protection should be applied. If the {@link RequestMatcher} returns true for
	 * a given request, then CSRF protection is applied.
	 *
	 * <p>
	 * The default is to apply CSRF protection for any HTTP method other than GET,
	 * HEAD, TRACE, OPTIONS.
	 * </p>
	 *
	 * @param requireCsrfProtectionMatcher the {@link RequestMatcher} used to
	 *                                     determine if CSRF protection should be
	 *                                     applied.
	 */
