 ...
 * 
 * HystrixConcurrencyStrategy：获取并发相关类 HystrixEventNotifier：事件通知类
 * HystrixMetricsPublisher：度量信息类 HystrixPropertiesStrategy：Properties配置类
 * HystrixCommandExecutionHook：HystrixCommand回调函数类 HystrixDynamicProperties：配置信息
 ...
 * class</li>
 * <li>plugin registered and retrieved using the resolved
 * {@link HystrixDynamicProperties} (usually Archaius, see get methods for
 * property names)</li>
 * <li>plugin registered and retrieved using the JDK {@link ServiceLoader}</li>
 * <li>default implementation</li>
 * </ol>
 * 
 * The exception to the above order is the {@link HystrixDynamicProperties}
 * implementation which is only loaded through <code>System.properties</code> or
 * the ServiceLoader (see the {@link HystrixPlugins#getDynamicProperties()
 * getter} for more details).
 * <p>
 * See the Hystrix GitHub Wiki for more information: <a href=
 * "https://github.com/Netflix/Hystrix/wiki/Plugins">https://github.com/Netflix/Hystrix/wiki/Plugins</a>.
 */
public class HystrixPlugins {

	// We should not load unless we are requested to. This avoids accidental
	// initialization. @agentgt
	...
	private HystrixPlugins(ClassLoader classLoader, LoggerSupplier logSupplier) {
		...
		// N.B. Do not use a logger before this is loaded as it will most likely load
	...
	/**
	 * Reset all of the HystrixPlugins to null. You may invoke this directly, or it
	 * also gets invoked via <code>Hystrix.reset()</code>
	 */
	...
	/**
	 * Retrieve instance of {@link HystrixEventNotifier} to use based on order of
	 * precedence as defined in {@link HystrixPlugins} class header.
	 * <p>
	 * Override default by using
	 * {@link #registerEventNotifier(HystrixEventNotifier)} or setting property (via
	 * Archaius): <code>hystrix.plugin.HystrixEventNotifier.implementation</code>
	...
	public HystrixEventNotifier getEventNotifier() {
		...
		if (notifier.get() == null) {
			...
			if (impl == null) {
				...
				// we don't return from here but call get() again in case of thread-race so the
				// winner will always get returned
			} else {
	...
	/**
	 * Register a {@link HystrixEventNotifier} implementation as a global override
	 * of any injected or default implementations.
	 * 
	 * @param impl {@link HystrixEventNotifier} implementation
	 * @throws IllegalStateException if called more than once or after the default
	 *                               was initialized (if usage occurs before trying
	 *                               to register)
	 */
	...
	/**
	 * Retrieve instance of {@link HystrixConcurrencyStrategy} to use based on order
	 * of precedence as defined in {@link HystrixPlugins} class header.
	 * <p>
	 * Override default by using
	 * {@link #registerConcurrencyStrategy(HystrixConcurrencyStrategy)} or setting
	 * property (via Archaius):
	 * <code>hystrix.plugin.HystrixConcurrencyStrategy.implementation</code> with
	...
	/**
	 * Register a {@link HystrixConcurrencyStrategy} implementation as a global
	 * override of any injected or default implementations.
	 * 
	...
	/**
	 * Retrieve instance of {@link HystrixMetricsPublisher} to use based on order of
	 * precedence as defined in {@link HystrixPlugins} class header.
	 * <p>
	 * Override default by using
	 * {@link #registerMetricsPublisher(HystrixMetricsPublisher)} or setting
	 * property (via Archaius):
	 * <code>hystrix.plugin.HystrixMetricsPublisher.implementation</code> with the
	...
	/**
	 * Register a {@link HystrixMetricsPublisher} implementation as a global
	 * override of any injected or default implementations.
	 * 
	...
	/**
	 * Retrieve instance of {@link HystrixPropertiesStrategy} to use based on order
	 * of precedence as defined in {@link HystrixPlugins} class header.
	 * <p>
	 * Override default by using
	 * {@link #registerPropertiesStrategy(HystrixPropertiesStrategy)} or setting
	 * property (via Archaius):
	 * <code>hystrix.plugin.HystrixPropertiesStrategy.implementation</code> with the
	 * full classname to load.
	 * 
	 ...
	 * <p>
	 * Unlike the other plugins this plugin cannot be re-registered and is only
	 * loaded at creation of the {@link HystrixPlugins} singleton.
	 * <p>
	 * The order of precedence for loading implementations is:
	 * <ol>
	 * <li>System property of key:
	 * <code>hystrix.plugin.HystrixDynamicProperties.implementation</code> with the
	 * class as a value.</li>
	 * <li>The {@link ServiceLoader}.</li>
	 * <li>An implementation based on Archaius if it is found in the classpath is
	 * used.</li>
	 * <li>A fallback implementation based on the
	...
	/**
	 * Register a {@link HystrixPropertiesStrategy} implementation as a global
	 * override of any injected or default implementations.
	 * 
	...
	/**
	 * Retrieve instance of {@link HystrixCommandExecutionHook} to use based on
	 * order of precedence as defined in {@link HystrixPlugins} class header.
	 * <p>
	 * Override default by using
	 * {@link #registerCommandExecutionHook(HystrixCommandExecutionHook)} or setting
	 * property (via Archaius):
	 * <code>hystrix.plugin.HystrixCommandExecutionHook.implementation</code> with
	 * the full classname to load.
	 * 
	...
	/**
	 * Register a {@link HystrixCommandExecutionHook} implementation as a global
	 * override of any injected or default implementations.
	 * 
	...
	private static <T> T getPluginImplementationViaProperties(Class<T> pluginClass,
			HystrixDynamicProperties dynamicProperties) {
		...
		if (implementingClass != null) {
			try {
				...
				throw new RuntimeException(classSimpleName + " implementation is not an instance of " + classSimpleName
						+ ": " + implementingClass);
			} catch (ClassNotFoundException e) {
				throw new RuntimeException(classSimpleName + " implementation class not found: " + implementingClass,
						e);
...
//	从System系统属性里找"hystrix.plugin." + classSimpleName + ".implementation"，也就是hystrix.plugin.HystrixDynamicProperties.implementation这个key。若存在就Class.forName()
//	通过ServiceLoader的SPI方式加载HystrixDynamicProperties的实现类
//	通过HystrixArchaiusHelper.createArchaiusDynamicProperties()创建一个实现类： 
//	让hystrix-plugins.properties和hystrix-plugins-环境名.properties里的属性都加入到全局配置中
//	从类路径下Class.forName("com.netflix.hystrix.strategy.properties.archaius.HystrixDynamicPropertiesArchaius")
//	这样Hystrix的属性就和全局属性关联上了喽，并且使用Archaius动态管理
//	最后，若都还没找到，那就兜底使用HystrixDynamicPropertiesSystemProperties：使用System属性（一般情况下，到第三步肯定就能完成实例化了）
	...
	private static HystrixDynamicProperties resolveDynamicProperties(ClassLoader classLoader,
		...
		if (hp != null) {
			logSupplier.getLogger()
					.debug("Created HystrixDynamicProperties instance from System property named "
		...
		if (hp != null) {
			logSupplier.getLogger().debug(
					"Created HystrixDynamicProperties instance by loading from ServiceLoader. Using class: {}",
					hp.getClass().getCanonicalName());
			return hp;
		}
