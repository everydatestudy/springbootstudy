 ...
 * <p>
 * This object can be referenced and "modified" by parent and child threads as
 * well as by different instances of HystrixCommand since 1 instance could
 * create an ExecutionResult, cache a Future that refers to it, a 2nd instance
 * execution then retrieves a Future from cache and wants to append
 * RESPONSE_FROM_CACHE to whatever the ExecutionResult was from the first
 * command execution.
 * <p>
 * This being immutable forces and ensure thread-safety instead of using
 * AtomicInteger/ConcurrentLinkedQueue and determining when it's safe to mutate
 * the object directly versus needing to deep-copy clone to a new instance.
 */
public class ExecutionResult {
	...
	private final int userThreadLatency; // time elapsed between caller thread submitting request and response being
											// visible to it
	...
	public static class EventCounts {
		...
		// numEmissions：对应HystrixEventType.EMIT总数
		// 执行command命令，在doOnNext时发送此事件
		private final int numEmissions;
		//numFallbackEmissions：对应HystrixEventType.FALLBACK_EMIT总数 
		...
		public String toString() {
			return "EventCounts{" + "events=" + events + ", numEmissions=" + numEmissions + ", numFallbackEmissions="
					+ numFallbackEmissions + ", numCollapsed=" + numCollapsed + '}';
		}
	}

	private ExecutionResult(EventCounts eventCounts, long startTimestamp, int executionLatency, int userThreadLatency,
			Exception failedExecutionException, Exception executionException, boolean executionOccurred,
			boolean isExecutedInThread, HystrixCollapserKey collapserKey) {
	...
	public ExecutionResult markCollapsed(HystrixCollapserKey collapserKey, int sizeOfBatch) {
		return new ExecutionResult(eventCounts.plus(HystrixEventType.COLLAPSED, sizeOfBatch), startTimestamp,
				executionLatency, userThreadLatency, failedExecutionException, executionException, executionOccurred,
	...
	public ExecutionResult markUserThreadCompletion(long userThreadLatency) {
		if (startTimestamp > 0 && !isResponseRejected()) {
			/*
			 * execution time (must occur before terminal state otherwise a race condition
			 * can occur if requested by client)
			 */
	...
	/**
	 * Creates a new ExecutionResult by adding the defined 'event' to the ones on
	...
	public ExecutionResult addEvent(HystrixEventType eventType) {
		return new ExecutionResult(eventCounts.plus(eventType), startTimestamp, executionLatency, userThreadLatency,
				failedExecutionException, executionException, executionOccurred, isExecutedInThread, collapserKey);
	}
	...
	@Override
	public String toString() {
		return "ExecutionResult{" + "eventCounts=" + eventCounts + ", failedExecutionException="
				+ failedExecutionException + ", executionException=" + executionException + ", startTimestamp="
				+ startTimestamp + ", executionLatency=" + executionLatency + ", userThreadLatency=" + userThreadLatency
				+ ", executionOccurred=" + executionOccurred + ", isExecutedInThread=" + isExecutedInThread
