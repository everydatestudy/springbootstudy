...
public final class RequestTemplate implements Serializable {
	 ...
	 * @param methodMetadata   containing a reference to the method this template is
	 *                         built from.
	 */
	private RequestTemplate(String target, String fragment, UriTemplate uriTemplate, BodyTemplate bodyTemplate,
			HttpMethod method, Charset charset, Request.Body body, boolean decodeSlash,
			CollectionFormat collectionFormat, MethodMetadata methodMetadata, Target<?> feignTarget) {
		this.target = target;
	...
	public static RequestTemplate from(RequestTemplate requestTemplate) {
		RequestTemplate template = new RequestTemplate(requestTemplate.target, requestTemplate.fragment,
				requestTemplate.uriTemplate, requestTemplate.bodyTemplate, requestTemplate.method,
				requestTemplate.charset, requestTemplate.body, requestTemplate.decodeSlash,
				requestTemplate.collectionFormat, requestTemplate.methodMetadata, requestTemplate.feignTarget);
  ...
  // 注意：变量值是一份，所有的模版都会解析哦
	 ...
	 *
	 * @param variables containing the variable values to use when resolving
	 *                  expressions.
  ...
  // 这里会解析UriTemplate，并且拼接好所有的部分（包括查询参数、锚点等，但domain此处不管哦~)
	 ...
	 * @param collectionFormat to use when resolving collection based query
	 *                         variables.
	 * @return a RequestTemplate for chaining.
	 */
