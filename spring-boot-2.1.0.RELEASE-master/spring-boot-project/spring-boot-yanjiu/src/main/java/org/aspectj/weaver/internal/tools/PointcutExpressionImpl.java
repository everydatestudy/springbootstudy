...
public class PointcutExpressionImpl implements PointcutExpression {
	 ...
	 * 
	 * @see
	 * org.aspectj.weaver.tools.PointcutExpression#setMatchingContext(org.aspectj.
	 * weaver.tools.MatchingContext)
	 */
	...
	public ShadowMatch matchesConstructorExecution(Constructor aConstructor) {
		...
		if (MATCH_INFO && match.maybeMatches()) {
			System.out.println("MATCHINFO: constructor execution match on '" + aConstructor + "' for '"
					+ this.expression + "': " + (match.alwaysMatches() ? "YES" : "MAYBE"));
		}
		return match;
	}
	...
	public ShadowMatch matchesStaticInitialization(Class aClass) {
		...
		if (MATCH_INFO && sm.maybeMatches()) {
			System.out.println("MATCHINFO: static initialization match on '" + aClass.getName() + "' for '"
	...
	public ShadowMatch matchesMethodCall(Method aMethod, Class callerType) {
		...
		if (MATCH_INFO && sm.maybeMatches()) {
			System.out.println("MATCHINFO: method call match on '" + aMethod + "' callerType='" + callerType.getName()
	...
	public ShadowMatch matchesConstructorCall(Constructor aConstructor, Class callerType) {
		...
		if (MATCH_INFO && sm.maybeMatches()) {
			System.out.println(
					"MATCHINFO: constructor call match on '" + aConstructor + "' callerType='" + callerType.getName()
							+ "' for '" + this.expression + "': " + (sm.alwaysMatches() ? "YES" : "MAYBE"));
		}
		return sm;
	}
	...
	public ShadowMatch matchesConstructorCall(Constructor aConstructor, Member withinCode) {
		...
		sm.setWithinType(withinCode.getDeclaringClass());
		if (MATCH_INFO && sm.maybeMatches()) {
			System.out.println("MATCHINFO: constructor call match on '" + aConstructor + "' withinCode='" + withinCode
					+ "' for '" + this.expression + "': " + (sm.alwaysMatches() ? "YES" : "MAYBE"));
		}
		return sm;
	}
	...
	public ShadowMatch matchesHandler(Class exceptionType, Class handlingType) {
		...
		if (MATCH_INFO && sm.maybeMatches()) {
			System.out.println("MATCHINFO: handler match on '" + exceptionType.getName() + "' handlingType='"
					+ handlingType + "' for '" + this.expression + "': " + (sm.alwaysMatches() ? "YES" : "MAYBE"));
		}
		return sm;
	}
	@SuppressWarnings("unused")
	...
	public ShadowMatch matchesFieldGet(Field aField, Class withinType) {
		...
		if (MATCH_INFO && sm.maybeMatches()) {
			System.out.println("MATCHINFO: field get match on '" + aField + "' withinType='" + withinType.getName()
					+ "' for '" + this.expression + "': " + (sm.alwaysMatches() ? "YES" : "MAYBE"));
		}
		return sm;
	}
	...
	public ShadowMatch matchesFieldSet(Field aField, Class withinType) {
		...
		sm.setWithinType(withinType);
		if (MATCH_INFO && sm.maybeMatches()) {
			System.out.println("MATCHINFO: field set match on '" + aField + "' withinType='" + withinType.getName()
					+ "' for '" + this.expression + "': " + (sm.alwaysMatches() ? "YES" : "MAYBE"));
		}
		return sm;
	}
