...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	public void refresh() throws BeansException, IllegalStateException {
		synchronized (this.startupShutdownMonitor) {
			...
			/**
			 * 表示真正做refresh操作之前需要准备做的事情： 1. 设置Spring容器的启动时间，撤销关闭状态，开启活跃状态 2. 初始化property 3.
			 * 验证环境信息里必须存在的属性
			 */
			prepareRefresh();
	...
	protected void prepareRefresh() {
		...
		/**
		 * 创建一个容器用于保存早期待发布的事件集合 什么是早期事件了? 就是我们的事件监听器还没有注册到事件多播器上的时候都称为早期事件
		 */
	...
	/**
	 *
	 * 配置其标准的特征，比如上下文的加载器ClassLoader和post-processors回调 Configure the factory's
	 * standard context characteristics, such as the context's ClassLoader and
	 * post-processors.
	 * 
	...
	protected void prepareBeanFactory(ConfigurableListableBeanFactory beanFactory) {
		 ...
		 * 那么当然需要在Spring做bean的依赖注入的时候忽略它们。 这个就是ignoreDependencyInterface的作用 /** 取消
