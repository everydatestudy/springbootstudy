 ...
 *
 * <p>
 * Delegates to {@link #processEvent(ApplicationEvent)} to give sub-classes a
 * chance to deviate from the default. Unwraps the content of a
 * {@link PayloadApplicationEvent} if necessary to allow method declaration to
 * define any arbitrary event type. If a condition is defined, it is evaluated
 * prior to invoking the underlying method.
 * 
 * DefaultListableBeanFactory#中preInstantiateSingletons->
 * (beanName为org.springframework.context.event.internalEventListenerProcessor
  *    时得到EventListenerMethodProcessor)EventListenerMethodProcessor#afterSingletonsInstantiated
 * this.processBean(factories,beanName,type)*然后把要执行的方法封装为ApplicationListenerMethodAdapter->添加到listener中
...
public class ApplicationListenerMethodAdapter implements GenericApplicationListener {
	 ...
	 * Can return {@code null} to indicate that no suitable arguments could be
	 * resolved and therefore the method should not be invoked at all for the
	 * specified event.
	 */
	@Nullable
	 ...
	 * annotation or any matching attribute on a composed annotation that is
	 * meta-annotated with {@code @EventListener}.
	 */
	@Nullable
	...
	/**
	 * Add additional details such as the bean type and method signature to the
	 * given error message.
	 * 
