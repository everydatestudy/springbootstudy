 ...
 * 
 * 绝大多数情况下，我都建议返回ModelAndView，而不是其它那哥三。因为它哥三都没有指定视图名，
 * 所以通过DispatcherServlet.applyDefaultViewName()生成的视图名一般都不是我们需要的。
 * （除非你的目录、命名等等都特别特别的规范，那顺便倒是可以省不少事~~~）
 *
 *
 * @author Rod Johnson
...
public class ModelAndView {
	...
	/**
	 * Default constructor for bean-style usage: populating bean properties instead
	 * of passing in constructor arguments.
	 * 
	 * @see #setView(View)
	 ...
	 *                 Model entries may not be {@code null}, but the model Map may
	 *                 be {@code null} if there is no model data.
	 */
	...
	/**
	 * Create a new ModelAndView given a View object and a model. <em>Note: the
	 * supplied model data is copied into the internal storage of this class. You
	 * should not consider to modify the supplied Map after supplying it to this
	 * class</em>
	 * 
	 * @param view  the View object to render
	 ...
	 * @param viewName name of the View to render, to be resolved by the
	 *                 DispatcherServlet's ViewResolver
	 * @param status   an HTTP status code to use for the response (to be set just
	...
	/**
	 * 总共这几个属性：它提供的构造函数非常的多 这里我就不一一列出 Set a view name for this ModelAndView, to be
	 * resolved by the DispatcherServlet via a ViewResolver. Will override any
	 ...
	 * ViewResolver, or {@code null} if we are using a View object.
	 */
	@Nullable
	...
	/**
	 * Set a View object for this ModelAndView. Will override any pre-existing view
	 * name or View.
	 */
	...
	/**
	 * Return the View object, or {@code null} if we are using a view name to be
	...
	/**
	 * Indicate whether or not this {@code ModelAndView} has a view, either as a
	...
	/**
	 * Return whether we use a view reference, i.e. {@code true} if the view has
	 * been specified via a name to be resolved by the DispatcherServlet via a
	 * ViewResolver.
	 */
	...
	/**
	 * Return the model map. May return {@code null}. Called by DispatcherServlet
	...
	/**
	 * Return the model map. Never returns {@code null}. To be called by application
	 ...
	 * 
	 * @param attributeName  name of the object to add to the model (never
	 *                       {@code null})
	...
	/**
	 * Return whether this ModelAndView object is empty, i.e. whether it does not
	 ...
	 * {@link #clear} i.e. whether it does not hold any view and does not contain a
	 * model.
	 * <p>
